...
def _install(command: str, settings_path: Path) -> None:
    ...
    claude_json.setdefault("mcpServers", {})["bonsai-python"] = {
        "type": "stdio",
        "command": command,
    ...
    # Write mcp__bonsai_python__* permission to ~/.claude/settings.json
    settings: dict = {}
    ...
    if "mcp__bonsai_python__*" not in allow:
        allow.append("mcp__bonsai_python__*")
        tmp = settings_path.with_suffix(".tmp")
        tmp.write_text(json.dumps(settings, indent=2) + "\n")
        os.replace(tmp, settings_path)

    print(f"Installed. Added 'bonsai-python' MCP server to {claude_json_path}")
    print("Restart Claude Code to load the bonsai-python MCP server.")
    print()
...
def _verify(settings_path: Path) -> None:
    ...
    if not claude_json_path.exists():
        ...
        try:
            ...
            data = {}
        if "bonsai-python" in data.get("mcpServers", {}):
            print(f"✓ MCP server registered in {claude_json_path}")
        else:
            print(f"✗ 'bonsai-python' not in {claude_json_path} — run: python -m bonsai_python --install")
            ok = False
    ...
    if not settings_path.exists():
        ...
        if "mcp__bonsai_python__*" in allow:
            print(f"✓ Permissions granted in {settings_path}")
        else:
            print(f"✗ 'mcp__bonsai_python__*' not in {settings_path} — run: python -m bonsai_python --install")
            ok = False
...
def main() -> None:
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"%(prog)s {importlib.metadata.version('bonsai-python')}",
    )
