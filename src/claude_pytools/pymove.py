...
def resolve_relative_import(importing_file: Path, root: Path, level: int, module: Optional[str]) -> Optional[str]:
    ...
    if not (importing_file.name == "__init__.py"):
        parts = parts[:-1]
...
def rewrite_import_line(
    ...
    if imp.kind == "import":
        ...
        if imp.level > 0:
            ...
            if resolved:
                ...
                if imp.module is None:
                    ...
                    for name, asname in imp.names:
                        ...
                        if candidate and (candidate == old_module or (old_package and candidate.startswith(old_package + "."))):
            ...
            names_str = ", ".join(
                f"{n} as {a}" if a else n for n, a in imp.names
            )
...
def plan_move(
    ...
    if is_package:
        ...
        new_module = path_to_module(dst / "__init__.py", root) if (dst / "__init__.py").exists() else ".".join(
            dst.resolve().relative_to(root).parts
        )
